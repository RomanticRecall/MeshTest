...
{
    // 基本运算符重载
    friend Real operator+(const Real &, const Real &); // 加法重载
    friend Real operator-(const Real &, const Real &); // 减法重载
    friend Real operator*(const Real &, const Real &); // 乘法重载
    friend Real operator/(const Real &, const Real &); // 除法重载
    friend Real operator-(const Real &);                 // 负号重载

    // 比较重载
    friend bool operator==(const Real &, const Real &); // 等于重载
    friend bool operator!=(const Real &, const Real &); // 不等于重载
    friend bool operator<(const Real &, const Real &);  // 小于重载
    friend bool operator<=(const Real &, const Real &); // 小于等于重载
    friend bool operator>(const Real &, const Real &);  // 大于重载
    friend bool operator>=(const Real &, const Real &); // 大于等于重载

    // 扩展运算符重载
    friend Real operator+=(Real &, const Real &); // 加等重载
    friend Real operator-=(Real &, const Real &); // 减等重载
    friend Real operator*=(Real &, const Real &); // 乘等重载
    friend Real operator/=(Real &, const Real &); // 除等重载

    // 输入输出重载
    friend ostream &operator<<(ostream &, const Real &); // 输出重载
    friend istream &operator>>(istream &, Real &);       // 输入重载


public:
    Real();
    Real(int);                          // 用一个整数构造
    Real(double);                       // 用一个浮点数构造
    Real(const string &);               // 用一个字符串构造
    Real(const Real &);               // 用一个高精度数构造
...
#define WFLOAT_ZERO Real::ZERO()
#define WFLOAT_ONE Real::ONE()
...
inline Real::Real(int num) // 用整型初始化
{
    if (num >= 0) // 判断正负
    {
        tag = true;
...
inline Real::Real(const string &num) // 用字符串初始化，格式形如"-123.456"、"1.0"
{
    // 用于判断小数与整数部分交界
...
inline Real::Real(const Real &num) // 利用高精度类初始化
{
    integer = num.integer;
    decimal = num.decimal;
    tag = num.tag;
...
inline Real::Real(Real &&num) noexcept // 移动构造
{
...
inline Real Real::operator=(const Real &num) // 赋值（拷贝）操作
{
    integer = num.integer;
    decimal = num.decimal;
    tag = num.tag;
...
inline Real Real::operator=(Real &&num) noexcept
{
    integer.swap(num.integer);
    decimal.swap(num.decimal);
    tag = num.tag;
...
inline Real Real::abs() const // 取绝对值
{
    if (tag)
        return (*this);
    else
...
inline Real Real::pow(int n) const
{
    Real ans = *this;
    for (int i = 1; i < n; i++)
    {
...
inline string Real::toString(size_t decimalNum) const
{
    string ans = "";
    Real temp = *this;
    if (!tag)
...
inline int Real::toInt() const // 转换为int
{
    return stoi(toString());
}

inline long Real::toLong() const // 转换为long
{
    return stol(toString());
}

inline long long Real::toLongLong() const // 转换为long long
{
    return stoll(toString());
}

inline float Real::toFloat() const // 转换为float
{
    return stof(toString());
}

inline double Real::toDouble() const // 转换为double
{
    return stod(toString());
}

inline long double Real::toLongDouble() const // 转换为long double
{
    return stold(toString());
}

inline Real operator-(const Real &num) // 取负操作
{
    Real temp(num);
    temp.tag = !temp.tag;
    return temp;
...
inline ostream &operator<<(ostream &out, const Real &num) // 输出重载
{
    if (!num.tag) // 负数
    {
        out << "-";
...
inline istream &operator>>(istream &in, Real &num) // 输入重载
{
    string str;
    in >> str;
    num = Real(str);
...
inline Real operator+=(Real &num1, const Real &num2) // 加等于重载
{
...
inline Real operator-=(Real &num1, const Real &num2) // 减等于重载
{
...
inline Real operator*=(Real &num1, const Real &num2) // 乘等于重载
{
    Real result(0);                               // 储存结果
...
inline Real operator/=(Real &num1, const Real &num2) // 除等于重载
{
    if (num2 == WFLOAT_ZERO)
...
inline Real operator+(const Real &num1, const Real &num2) // 调用+=
{
    Real temp(num1);
    temp += num2;
    return temp;
...
inline bool operator<(const Real &num1, const Real &num2) // 小于重载
{
    bool sign;                // 返回值
...
inline bool operator>(const Real &num1, const Real &num2) // 大于重载
{
    bool tag = !(num1 <= num2);
    return tag;
}

inline bool operator==(const Real &num1, const Real &num2) // 等于重载
{
...
inline bool operator!=(const Real &num1, const Real &num2)
{
    return !(num1 == num2);
}
